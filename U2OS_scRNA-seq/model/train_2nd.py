...
def train_dynamics(args, encoder, decoder, dynamics_learner, optimizer, scheduler, device, train_loader, use_cuda):
    ...
    for step in range(1, args.sub_epochs + 1):
        ...
        for batch_idx, data in enumerate(train_loader):
            data = data.to(device)
            loss, loss1, loss2, loss3 = train_dynamics_learner(optimizer, encoder, decoder, dynamics_learner,
        ...
        print('\nTraining %d/%d before validation, loss: %f, loss1: %f, loss2: %f, loss3: %f'
              % (step, args.sub_epochs, loss_out, loss1_out, loss2_out, loss3_out))
        print(f'Current learning rate: {current_lr}')
        scheduler.step()
        time4 = time.time()
        print("it spends %d seconds for trainsing this sub-epoch" % (time4-time3))

    return loss_records, loss1_records, loss2_records, loss3_records
...
def train_dynamics_learner(optimizer, encoder, decoder, dynamics_learner, data, steps, use_cuda, device):
    ...
    encoded_z = (encoded - latent_mean)/latent_std
    ...
    latent_t = odeint(dynamics_learner, encoded_z, t, method='rk4')
    latent_t = latent_t[1:, :, :]
    ...
    loss3 = F.l1_loss(latent_t, latent_mu, reduction='sum') / encoded.size(1) ## 这里可能要改成latent_mu
    loss = loss3
...
with open('np_data.pickle', 'rb') as f:
    np_data0 = pickle.load(f)
np_data0 = np_data0[:, ::2]
np_data = (np_data0 - np_data0.min(0)) / (np_data0.max(0) - np_data0.min(0))
raw = torch.as_tensor(np_data, dtype=torch.float32).to(device)
...
params = list(dynamics_learner.parameters())
optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, params), lr=1e-3, weight_decay=1e-6)
...
for epoch in range(1, args.epochs + 1):
    ...
    out_loss, loss1, loss2, loss3 = train_dynamics(args, encoder, decoder, dynamics_learner, optimizer, scheduler, device, train_loader, use_cuda)
