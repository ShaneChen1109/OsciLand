...
b1 = -1.4; u1 = 2.2; b2 = -1.7; u2 = 2.2; 
% b1 = -7; u1 = 4; b2 = -5; u2 = 5;
D = 0.2; % 扩散系数
scatters = zeros(1040,3);
...
% [~,l1] = findpeaks(-Z);
% [~,l2] = findpeaks(Z);
% Barrier_1 = Z(l2(1)) - Z(l1(1));
% Barrier_2 = Z(l2(2)) - Z(l1(2));
% Barrier_3 = Z(l2(3)) - Z(l1(3));
% c1 = l1(1); %G0/G1 phase
% c2 = l1(2); %S phase
% c3 = l1(3); %G2/M phase
% c4 = l2(1); %G1 checkpoint
% c5 = l2(2); %G2 checkpoint
% c6 = l2(3); %M checkpoint
...
% text(circle(c1,:) * V(:,1), circle(c1,:) * V(:,2), Z(c1)+1,'G0/G1 phase', 'FontSize', 12, 'FontName', 'Arial');
% text(circle(c2,:) * V(:,1), circle(c2,:) * V(:,2), Z(c2)+1,'S phase', 'FontSize', 12, 'FontName', 'Arial');
% text(circle(c3,:) * V(:,1), circle(c3,:) * V(:,2), Z(c3)+1,'G2/M phase', 'FontSize', 12, 'FontName', 'Arial');

% plot3(circle(c4, :) * V(:, 1), circle(c4, :) * V(:, 2), Z(c4) + 0.1, '.', 'Color', [1.0, 0.6, 0.6], 'MarkerSize', 30);
% plot3(circle(c5, :) * V(:, 1), circle(c5, :) * V(:, 2), Z(c5) + 0.1, '.', 'Color', [1.0, 0.6, 0.6], 'MarkerSize', 30);
% plot3(circle(c6, :) * V(:, 1), circle(c6, :) * V(:, 2), Z(c6) + 0.1, '.', 'Color', [1.0, 0.6, 0.6], 'MarkerSize', 30);
% text(circle(c4,:) * V(:,1), circle(c4,:) * V(:,2), Z(c4)+1,'G1 checkpoint', 'FontSize', 12, 'FontName', 'Arial');
% text(circle(c5,:) * V(:,1), circle(c5,:) * V(:,2), Z(c5)+1,'G2 checkpoint', 'FontSize', 12, 'FontName', 'Arial');
% text(circle(c6,:) * V(:,1), circle(c6,:) * V(:,2), Z(c6)+1,'M checkpoint', 'FontSize', 12, 'FontName', 'Arial');
...
%     flux_proj(1:step:length(circle_proj),1),flux_proj(1:step:length(circle_proj),2),0*Z(1:step:length(circle_proj),1), ...
...
% zlim([8 11.6]);
colorbar
xlabel('latent3', 'FontSize', 52, 'FontName', 'Arial');
ylabel('latent5', 'FontSize', 52, 'FontName', 'Arial');
zlabel('U', 'FontSize', 52, 'FontName', 'Arial');
set(gca, 'FontSize', 25, 'FontName', 'Arial');
% set(gca, 'LineWidth', 1.5);
% view([-65 77])
view(2); % 用于平面视图
set(gcf,'outerposition', [100 100 570 500])
set(gcf, 'PaperPositionMode', 'auto')
% print(gcf, 'landscape.pdf', '-dpdf', '-painters')
...
% quiver3(a1(mg,ng),a2(mg,ng),13*ones(21,21), ...
...
quiver3(a1(mg,ng),a2(mg,ng),13*ones(21,21), ...
    15*Jx(mg,ng),15*Jy(mg,ng),0*JJx(mg,ng), ...
    1.0,'Color',[1,0.6,0.6],'LineWidth',1); % flux
...
colors = [
    0.8431, 0.0000, 0.0000;  % Phase 0 (#d70000)
    0.1843, 0.2431, 0.6588;  % Phase 1 (#2f3ea8)
    0.8549, 0.5882, 1.0000;  % Phase 2 (#da96ff)
    0.6784, 0.5804, 0.9255;  % Phase 3 (#ad94ec)
    0.6549, 0.4196, 0.6039;  % Phase 4 (#a76b9a)
];
category_indices = scatters(:,3);

scatter_colors = colors(category_indices+1, :);

% scatter3(scatters(:,1), scatters(:,2),U_s+0.2, 30, scatter_colors, 'filled', 'MarkerFaceAlpha', 0.6);
end
