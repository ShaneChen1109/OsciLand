...
x1 = torch.as_tensor(np_data[0, :].reshape(-1, 1066), dtype=torch.float32)
...
for i in range(NK):
    ...
    d_sigma = torch.matmul(sigma1, jac.t()) + torch.matmul(jac, sigma1) + 2 * 0.2 * torch.eye(20)  ## 这里改扩散系数D
    x1 = x1 + f * 0.005
...
# plt.plot(xx[2, 9125:9622], xx[4, 9125:9622], linewidth=2.0)
# plt.show()

## 1674个格点达成一个周期
# circle = xx[:, 9125:9457]
# sigmacell= sigma[:, :, 9125:9457]
circle = xx[:, 9125:9622]
sigmacell= sigma[:, :, 9125:9622]
# circle = xx[:, 9125:9869]
# sigmacell= sigma[:, :, 9125:9869]
